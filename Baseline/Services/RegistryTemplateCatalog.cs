...
public sealed class RegistryTemplateCatalog
{
    ...
    [
        new() { Id = "graphics.hardware-gpu", GroupName = "Graphics", DisplayName = "Hardware accelerated GPU scheduling", Root = RegistryRoot.LocalMachine, Path = @"SYSTEM\CurrentControlSet\Control\GraphicsDrivers", ValueName = "HwSchMode", SafetyLevel = SafetyLevel.Moderate, CompatibilityNote = "Requires supported GPU drivers.", IsDefaultSelected = true },
        ...
        new() { Id = "explorer.separate-process", GroupName = "Explorer/UI", DisplayName = "Launch folder windows in a separate process", Root = RegistryRoot.CurrentUser, Path = @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", ValueName = "SeparateProcess", SafetyLevel = SafetyLevel.Safe, IsDefaultSelected = true },
